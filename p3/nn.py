...
class NN:
    ...
    def compute_gradients(self, X, Y):
        ...
        for idx in range(num_layers-2, -1, -1):
            ...
            ### YOUR CODE HERE ###
            grad = self.activation_function.backprop_grad(
                np.matmul(ws[idx],D_stack[idx]) + bs[idx])*(np.matmul(ws[idx+1].T, grad))
